...
export class IndiAdjustmentService {
  ...
  async create(createIndiAdjustmentDto: Prisma.IndividualSalaryAdjustmentsCreateInput) {
    try {
      return await this.databaseService.individualSalaryAdjustments.create({ data: createIndiAdjustmentDto });
    } catch (error) {
  ...
  async findOne(id: number ) {
    try {
      const adjust = await this.databaseService.individualSalaryAdjustments.findUnique({
        where: { id },
      });
      if (!adjust) {
  ...
  async update(id: number, updateIndiAdjustmentDto: Prisma.IndividualSalaryAdjustmentsUpdateInput) {
    try {
